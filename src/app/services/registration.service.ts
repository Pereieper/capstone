...
export class RegistrationService {
  ...
  async checkLogin(contact: string, password: string): Promise<any | null> {
    ...
    const secretary = await storageInstance.get('secretary');
    const captain = await storageInstance.get('captain');

    if (secretary && secretary.phone === contact && secretary.password === password) {
      return { ...secretary, role: 'secretary' };
    }

    if (captain && captain.phone === contact && captain.password === password) {
